...
parser.add_argument("--ymin",type=float, help="set min value for yrange of limit vs duration plot")
parser.add_argument("--ymax",type=float, help="set max value for yrange of limit vs duration plot")
...
if hasdet:
    ...
    for a,Zrate in zip(axs[0:2],[llZrate,ulZrate]):
        lev_exp = np.linspace(np.log10(Zrate[(Zrate > 0) & (Zrate != np.inf)].min()),np.log10(Zrate[(Zrate > 0) & (Zrate != np.inf)].max()), num=1000)
        levs = np.power(10, lev_exp)
        csrate = a.contourf(10**X, 10**Y, Zrate, levels=levs, cmap='viridis', norm=colors.LogNorm(), vmin=Zrate[(Zrate > 0) & (Zrate != np.inf)].min(), vmax=Zrate[(Zrate > 0) & (Zrate != np.inf)].max())
        rateticks = np.geomspace(Zrate[(Zrate > 0) & (Zrate != np.inf)].min(),Zrate[(Zrate > 0) & (Zrate != np.inf)].max(),num=10)
        ...
        cbarrate = fig.colorbar(csrate, ticks=rateticks, format=ticker.StrMethodFormatter("{x:01.1e}"), ax=a)
        cbarrate.set_label('Transient Rate per day per sq. deg.')
    a = axs[2]
    a.set_title('Transient Rate Upper Limits')
    a.vlines(10**X[index,:],llZrate[index,:],ulZrate[index,:])
    a.set_xscale('log')
    a.set_yscale('log')
    thisll = llZrate[index,:]
    validvallower = thisll[~np.isnan(thisll) & ~np.isinf(thisll)]
    if not args.ymin:
        ymin = validvallower.min()
    else:
        ymin = args.ymin
    thisul = ulZrate[index,:]
    validvalupper = thisul[~np.isnan(thisul) & ~np.isinf(thisul)]
    if not args.ymax:
        ymax = validvalupper.max()
    else:
        ymax = args.ymax
