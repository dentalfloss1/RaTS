...
class fred:
    ...
    def lines(self, xs, ys, durmax, max_distance, flux_err, obs):
        ...
        for x in xs:
            ...
            try:
                maxdist_y =  np.append(maxdist_y, (((1. + flux_err) * sens_maxgap * duration_maxgap) /  np.power(10,x))   / (np.exp(-(max_distance / np.power(10,x))) - np.exp(-(max_distance + duration_maxgap) / np.power(10,x))))
            except:
