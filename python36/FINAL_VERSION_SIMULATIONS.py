...
# Observational Parameters, if simulated
    ...
    if lightcurvetype != 'tophat' and lightcurvetype != 'fred' and lightcurvetype != 'gaussian' and lightcurvetype != 'wilma' and lightcurvetype != 'ered' and lightcurvetype != 'halfgaussian1':
        print('Type of transient not recognised.\nUse tophat, fred, gaussian, wilma, ered or halfgaussian1.')
        exit()
...
def generate_sources(n_sources, file, start_time, end_time, fl_min, fl_max, dmin, dmax, dump_intermediate, lightcurve):
    ...
    elif((lightcurve == "tophat") or (lightcurve == "fred") or (lightcurve == "halfgaussian1")):
...
def plots(obs, file, extra_threshold, det_threshold, flux_err, lightcurve, toplot):
    ...
    if (lightcurve == 'fred'):
        ...
        for x in xs:
            try:
                durmax_y = np.append(durmax_y, ((1. + flux_err) * sens_last * day1_obs  ) / (gausscdf(np.power(10,2.0*x),durmax + np.power(10,x)) - gausscdf(np.power(10,2.0*x), durmax - day1_obs + np.power(10,x))))
            except:
                durmax_y = np.append(durmax_y, np.inf)
            try:
