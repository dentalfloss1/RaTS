...
def generate_sources(n_sources, file, start_time, end_time, fl_min, fl_max, dmin, dmax):
	...
	bursts[:,1] = np.power(10, np.random.uniform(np.log10(dmin), np.log10(dmax), n_sources))
	bursts[:,2] = np.power(10, np.random.uniform(np.log10(fl_min), np.log10(fl_max), n_sources))
